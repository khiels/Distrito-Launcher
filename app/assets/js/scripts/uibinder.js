...
function showMainUI(data){
    ...
    setTimeout(() => {
        document.getElementById('frameBar').style.backgroundColor = '#0C0C0C'
...
function showFatalStartupError(){
    setTimeout(() => {
        $('#loadingContainer').fadeOut(250, () => {
            ...
            setOverlayContent(
                'Erro Fatal: Não foi possivel carregar o index de distribuição',
                'Não foi possivel estabelecer uma conexão com os nossos servidores para baixar o index de distribuição. Nenhuma cópia local foi encontrada. <br><br>Certifique-se que você esta conectado a internet e reabra o launcher.',
                'Fechar'
            )
            setOverlayHandler(() => {
...
async function validateSelectedAccount(){
    ...
    if(selectedAcc != null && (selectedAcc.type == 'mojang' || selectedAcc.type == 'microsoft')){
        ...
        if(!val){
            ...
            setOverlayContent(
                'Falha ao atualizar o login',
                `Não conseguimos atualizar o login para o usuário <strong>${selectedAcc.displayName}</strong>. Por favor ${accLen > 0 ? 'selecione outra conta ou ' : ''} faça login novamente.`,
                'Login',
...
function setSelectedAccount(uuid){
    ...
    if(authAcc.type === 'mojang' || authAcc.type === 'microsoft')
        validateSelectedAccount()
    else 
