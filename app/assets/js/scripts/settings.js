...
function bindFileSelectors(){
    for(let ele of document.getElementsByClassName('settingsFileSelButton')){
        ...
        ele.onclick = async e => {
            ...
            if(!res.canceled) {
                ele.getElementsByClassName('settingsFileSelVal')[0].value = res.filePaths[0]
                if(isJavaExecSel) {
                    populateJavaExecDetails(ele.getElementsByClassName('settingsFileSelVal')[0].value)
                }
            }
        }
    }
...
function bindAuthAccountSelect(){
    Array.from(document.getElementsByClassName('settingsAuthAccountSelect')).map((val) => {
        val.onclick = (e) => {
            ...
            const targetAccountUUID = val.closest('.settingsAuthAccount').getAttribute('uuid')
            const targetAccountUsername = val.closest('.settingsAuthAccount').getAttribute('username')
            const targetAcc = ConfigManager.getAuthAccount(targetAccountUUID) || ConfigManager.getAuthAccount(targetAccountUsername)
            setSelectedAccount(
                targetAcc.type === 'mojang' || targetAcc.type === 'microsoft' ? targetAccountUUID : targetAccountUsername)
        }
    })
}
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Alerta!<br>Esta é sua ultima conta',
                    'Para usar o launcher você precisa estar conectado em pelo menos uma conta. Após deslogar você terá que fazer login novamente.<br><br>Deseja continuar?',
                    'Continuar',
                    'Cancelar'
                )
                setOverlayHandler(() => {
...
function processLogOut(val, isLastAccount){
    ...
    const username = parent.getAttribute('username')
    const prevSelAcc = ConfigManager.getSelectedAccount()
    const targetAcc = ConfigManager.getAuthAccount(uuid) || ConfigManager.getAuthAccount(username)
    if(targetAcc.type === 'microsoft') {
        ...
        AuthManager.removeAccount(username)
        if(!isLastAccount && uuid === prevSelAcc.uuid) {
            const selAcc = ConfigManager.getSelectedAccount()
            refreshAuthAccountSelected(selAcc.uuid)
            updateSelectedAccount(selAcc)
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Algo deu errado',
                'A tentativa de deslogar falhou. Por favor tente novamente.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
...
function refreshAuthAccountSelected(uuid){
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if(uuid === val.getAttribute('uuid')){
            ...
            selBtn.innerHTML = 'Conta Selecionada &#10004;'
        } else {
            if(selBtn.hasAttribute('selected')){
                selBtn.removeAttribute('selected')
            }
            selBtn.innerHTML = 'Selecionar Conta'
        }
    })
}
...
function populateAuthAccounts(){
    ...
    authKeys.forEach((val) => {
        ...
        const accHtml = `<div class="settingsAuthAccount ${selectedUUID === acc.uuid ? 'activeAccount' : ''}" uuid="${acc.uuid}" username="${acc.username}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="https://mc-heads.net/body/${acc.type === 'mojang' || acc.type === 'microsoft' ? acc.uuid : acc.username}/60">
            </div>
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    ...
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Conta Selecionada &#10004;' : '>Selecionar Conta'}</button>
                </div>
            </div>
        </div>`
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Falha ao deletar<br>Drop-in Mod ${fullName}`,
                    'Tenha certeza que o arquivo não esta sendo usado e tente novamente.',
                    'Ok'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Falha ao Acionar<br>Um ou Mais Drop-in Mods',
                            err.message,
                            'Ok'
                        )
                        setOverlayHandler(null)
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Selecionado: Java ${v.version.major} Update ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Selecionado: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
            settingsJavaExecDetails.innerHTML = 'Seleção Inválida'
        }
    })
}
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Pré-atualização'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = 'Atualização'
...
const settingsUpdateChangelogText  = document.getElementById('changeLogText')
/**
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nova ${isPrerelease(data.version) ? 'Pre-atualização' : 'Atualização'} Disponivel`
        ...
        settingsUpdateButtonStatus('Checar atualizações', false, () => {
            if(!isDev){
